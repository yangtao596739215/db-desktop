...
func (s *SimpleDatabaseManager) executeMySQLQuery(connectionID string, query string) (*QueryResult, error) {
	fmt.Printf("[MySQL] ExecuteQuery called - ConnectionID: %s, Query: %s\n", connectionID, query)

	db, exists := s.mysqlConnections[connectionID]
	if !exists {
		fmt.Printf("[MySQL] Connection not found: %s\n", connectionID)
	...
	if err != nil {
		fmt.Printf("[MySQL] Query failed - ConnectionID: %s, Error: %v\n", connectionID, err)
	...
	if err != nil {
		fmt.Printf("[MySQL] Get columns failed - ConnectionID: %s, Error: %v\n", connectionID, err)
		return &QueryResult{Error: err.Error(), Time: time.Since(start).Milliseconds()}, err
	}

	fmt.Printf("[MySQL] Query columns: %v\n", columns)
	...
	for rows.Next() {
		...
		if err := rows.Scan(valuePtrs...); err != nil {
			fmt.Printf("[MySQL] Scan row failed - ConnectionID: %s, Error: %v\n", connectionID, err)
			return &QueryResult{Error: err.Error(), Time: time.Since(start).Milliseconds()}, err
		}
	...
	fmt.Printf("[MySQL] Query completed - ConnectionID: %s, Rows: %d, Time: %dms\n",
		connectionID, rowCount, time.Since(start).Milliseconds())
...
func (s *SimpleDatabaseManager) getMySQLTables(connectionID string, database string) ([]TableInfo, error) {
	fmt.Printf("getMySQLTables 调用: connectionID=%s, database=%s\n", connectionID, database)
